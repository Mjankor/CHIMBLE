...
double Sensible;
double Latent;
	//Rcpp::Rcout << "Running LatSens Function" << std::endl;
...
LakeSpecHumidity = 0.622 * (LakeSatVapourPressure/(SurfacePressure*100 - 0.378*LakeSatVapourPressure));
	//Rcpp::Rcout << "AirSpecHumidity: " << AirSpecHumidity << ", LakeSpecHumidity: " <<LakeSpecHumidity << std::endl;
//described here (p65)
//https://books.google.com.au/books?id=nWr1AwAAQBAJ&pg=PA65&lpg=PA65&dq=specific+humidity+622+378&source=bl&ots=MjpE9qBVFb&sig=ACfU3U2tvAgQmU00FgSOHrXyMRNs8TcqcQ&hl=en&sa=X&ved=2ahUKEwilq7Xh1KzjAhWSguYKHV0vAmMQ6AEwAHoECBMQAQ#v=onepage&q=specific%20humidity%20622%20378&f=false
...
Days = EndDay - StartDay;
	for (int i = 0; i < Days; i++){
		...
		//Note. This is the same code as the Feedback function in hydrology, but is kept separate here to take advantage of the interpolation routines. At some stage we should investigate combining feedback and the interpolation routines. 
		//Rcpp::Rcout << "Index: " << i << ", RH " << RH << ", ";
		RH = RH * (1-Feedback) + Feedback;
		if (RH > 1){
		RH = 1;
		}
		//Rcpp::Rcout << "RHfeedback " << RH << std::endl;
		...
		//Rcpp::Rcout << "Running Temp Profile" << std::endl;
		...
		//Rcpp::Rcout << "Running mixing" << std::endl;
