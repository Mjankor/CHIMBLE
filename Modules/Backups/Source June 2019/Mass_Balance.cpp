...
// [[Rcpp::export]]
void func_FluxE (NumericMatrix Flux, long Time, NumericMatrix Met, double ALBlake, double PWF, double LatitudeRadians, std::string Datarate, double Timestep, NumericMatrix ChemConcentrations, std::string StratificationModule, double NeutralDragCoeff){
EvapRate(0) = func_E(Time, Met, ALBlake, PWF, LatitudeRadians, Datarate, Timestep, ChemConcentrations, StratificationModule, NeutralDragCoeff);
}
...
func_FluxE (Flux, Time, Met, ALBlake, PWF, LatitudeRadians, Datarate, Timestep, ChemConcentrations, StratificationModule, NeutralDragCoeff);
...
// [[Rcpp::export]]
void func_IsotopeFluxes (NumericMatrix Flux, long Time, NumericMatrix Isotopes, NumericMatrix Met, double AtmosphericShift, double Turbulence, double Feedback, NumericMatrix Lakevolumes, NumericMatrix Lake, std::string Interpolationtype, std::string DirectExternalFlows, NumericMatrix FluxD, NumericMatrix Flux18O, std::string GroundwaterModule, List GWStats, NumericMatrix GWNumbers, NumericMatrix ChemConcentrations){
    E_D(-1) = func_dE(Time-1, "D", Isotopes, Met, AtmosphericShift, Turbulence, Feedback, Lakevolumes, Lake, Interpolationtype, ChemConcentrations);	
	E_18O(-1) = func_dE(Time-1, "18O", Isotopes, Met, AtmosphericShift, Turbulence, Feedback, Lakevolumes, Lake, Interpolationtype, ChemConcentrations);
...
//Check for events		
	...
	//Rcpp::Rcout << "Date checked" << Met(Time,0) << "/" << Met(Time,1) << "/" << Met (Time,2) << std::endl;
		...
		if ((Met(Time,0) == Year(j)) && (Met(Time,1) == Month(j)) && (Met(Time,2) == Day(j))){
				...
				if (Names(j) == "AWCds") {AWCds  = ScenarioEventsList(j);}
