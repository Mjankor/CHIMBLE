...
Esa = 6.108 * exp(17.27 * Met(Time,4)/(Met(Time,4)+237.7));
...
//Rcpp::Rcout << "Act_Isotopes" << Act_Isotopes << std::endl;

Theta = (1- func_FeedbackRH(Met, Time, Feedback))/(1- func_FeedbackRH(Met, Time, 0));
EKin = CKinetic * (1-RH_Norm)*Theta * Turbulence;
