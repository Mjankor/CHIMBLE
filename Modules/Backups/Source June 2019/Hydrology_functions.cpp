...
// [[Rcpp::export]]
double func_E(long Time, NumericMatrix Met, double ALBlake, double PWF, double LatitudeRadians, std::string Datarate, double Timestep, NumericMatrix ChemConcentrations, std::string StratificationModule, double NeutralDragCoeff){
double E = 0;
...
if (StratificationModule == "Yes"){
	NumericVector Heat = func_LatSensHeat (Met(Time,4) + Met(Time,10), Met(Time,4), Met(Time,7), 0, 0, Met(Time,14), Met(Time,5)/100, NeutralDragCoeff);
	E = Heat(2); //mm per day
	E = E*Act_Water;
} else {
	if (Met(Time, 4) <= 0) {
...
// [[Rcpp::export]]
NumericVector func_Fe(long Time, double Timestep, NumericMatrix Lake, NumericMatrix Met, double ALBlake, NumericMatrix Isotopes, double PWF, double LatitudeRadians, std::string Datarate, double KcLwin, double KcLsum, NumericMatrix ChemConcentrations, std::string StratificationModule, double NeutralDragCoeff){
	double Fe;
	double Fe18O;
	double FeD;
	if (StratificationModule == "Yes"){
		Fe = func_E(Time, Met, ALBlake, PWF, LatitudeRadians, Datarate, Timestep, ChemConcentrations, StratificationModule, NeutralDragCoeff) * Lake(Time,6) * Timestep;
	} else {
		Fe = func_E(Time, Met, ALBlake, PWF, LatitudeRadians, Datarate, Timestep, ChemConcentrations, StratificationModule, NeutralDragCoeff) * Lake(Time,6) * func_KcLake(Time, KcLwin, KcLsum, Met,Datarate, Timestep) * Timestep;
	}
